...
def main():
    ...
    model = ISNN(args.num_classes, args.dataset, args.seg, args)
    ...
    scheduler = MultiStepLR(optimizer, milestones=[60, 90, 110,140], gamma=0.1)
    # Data loading
    ...
    if args.train == 1:
        for epoch in range(args.start_epoch, args.max_epochs):
            ...
            ####### store tensor in cpu
            current = torch.as_tensor(current).cpu()
            ...
            if monitor_op(current, best):
                ...
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                file = open(save_path + '/log.txt','a+')
                file.write('Epoch %d: %s:   %.4f  --------->  %.4f\n '
                      % (epoch + 1, args.monitor, best, current))
                file.close()
...
def test(test_loader, model, checkpoint, lable_path, pred_path):
    ...
    pred_output = pred_output.reshape(-1, args.num_classes)
